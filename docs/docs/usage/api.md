...
6) Select the file `postman-env.json` generated by the [Config Exporter](http://localhost:8081/docs/getting-started/developers/#local-development). The file wil be parsed to an environment with the name schema `SLM - <<your-slm-hostname>>`. Press the `Import` button to finally import the environment.
